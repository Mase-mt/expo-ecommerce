...
const ProductsGrid = ({products, isLoading, isError}: ProductsGridProps) => {
  ...
  const {isInWishlist, isAddingToWishlist, toggleWishlist, isRemovingFromWishlist} = useWishlist();
  ...
  const renderProduct = ({item:product}:{item: Product}) => (
    ...
    >
      <View className='relative'>
        ...
        className='w-full h-44 bg-background-light'
        resizeMode='cover'/>
        <TouchableOpacity
      ...
      <View className='p-3'>
        <Text className='text-text-secondary text-sm mb-1'>{product.category}</Text>
      ...
      <View className='flex-row items-center justify-between'>
          <Text className='text-primary text-lg font-bold'>{product.price.toFixed(2)}</Text>
          <TouchableOpacity
