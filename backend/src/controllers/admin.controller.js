...
export async function getAllProducts(req,res){
    try {
        //-1 means descending order, means most recent product first
...
export async function updateProduct(req,res){
    try {
        ...
        //handling image updates if new images are uploaded
        if(req.files && req.files.length > 0){
            if(req.files.length > 3){
...
export async function getDasboardStats(req,res){
    try {
    ...
    try {
        ...
        if(product.images && product.images.length > 0){
            const deletePromises = product.images.map((imageUrl) =>{
                //Extract public_id from URL (assumes ../products/publicId.ext)
        ...
        res.status(500).json({error:"Faild to delete product, Internal server error"});
    }
}